...
export function FeaturedProjectsSection() {
  return (
    <section className="border-y border-white/10 bg-[linear-gradient(180deg,#100d0c,#0b0908)] py-24 md:py-28">
      <Container>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          {projects.map((project, index) => (
            <GlassCard
              key={project.slug}
              className={cn(
                "group overflow-hidden transition hover:-translate-y-1 hover:border-gold/35",
            ...
            >
              <Link href={`/projects/${project.slug}`} className="grid h-full no-underline">
                <div
                  className={cn(
                    "relative h-[310px] overflow-hidden bg-ink md:h-[360px]",
                    index === 0 && "lg:h-[590px]",
                  )}
                >
                  <Image
